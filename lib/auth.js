...
var carn = function (param) {
    if (!param) return "";
    else return param.split("&").sort().join("&");
};
...
exports.sign = function (requestOptions, options) {
    ...
    requestOptions.headers.Authorization = "AWS4-HMAC-SHA256 " +
        "Credential=" + options.AccessKeyId + "/" + 
        requestOptions.headers.Date.substring(0,8) + "/" + options.Region + "/glacier/aws4_request," +
