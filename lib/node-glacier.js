...
exports.Client = function (options) {
    ...
    this.deleteArchive = function (vaultName, archiveId, callback) {
    ...
    this.initiateJob = function (name, reqPayload, callback) {
        ...
        auth.sign(requestOptions, options, crypto.createHash("sha256").update(JSON.stringify(reqPayload)).digest("hex"));
        // console.dir(requestOptions);
        try {
            http.request(requestOptions, function (res) {
                // console.info(res.statusCode);
 ...
 */
    this.uploadArchiveFromS3Stream = function (bucket, path, vaultName, callback) {
        ...
        s3Client.get(path).on("response", function (res) {
            ...
            if (res.statusCode !== 200) {
                ...
                var myTreeHash = require("treehash").createTreeHashStream();
                var linearHash = crypto.createHash("sha256");
                var filesize = 0, expectedFileSize = parseInt(res.headers["content-length"]);
                res.on("data", function (chunk) {
                    res.pause();
                    myTreeHash.update(chunk);
                    linearHash.update(chunk);
                    filesize += chunk.length;
                    res.resume();
                });
                res.on("end", function () {
                    if (filesize !== expectedFileSize) throw new Error("The file I received from S3 has " + filesize + " bytes, but S3 claims that it should have " + expectedFileSize + " bytes.")
                    var tHash = myTreeHash.digest ();
                    var lHash = linearHash.digest ("hex");

                    // console.info ("Tree hash = " + tHash);
                    // console.info ("Linear hash = " + lHash);
                    ...
                    // console.info("File size is " + filesize + " bytes.");
                    // console.info("Re-downloading the same file for upload.");
                    s3Client.get(path).on("response", function (resp) {
                        var req = http.request(requestOptions, function (res) {
                            // console.info(res.statusCode);
                            // res.on("data", function (chunk) {console.info("" + chunk)});
                            err = (res.statusCode !== 201);
        ...
        s3Client.get(path).on("response", function (res) {
            ...
            if (res.statusCode !== 200) {
                ...
                res.on("end", function () {
                    ...
                    var getTreeHash = require("treehash").getTreeHashFromBuffer;
                    var treeHash    = getTreeHash(file);
                    var linearHash  = crypto.createHash("sha256").update(file).digest("hex");
    ...
    this.batchUploadArchivesFromS3 = function (bucket, prefix, vaultName, callback) {
        ...
        var uploadArchiveFromS3 = this.uploadArchiveFromS3Stream;
        ...
        s3Client.list({"prefix":prefix}, function (err, data) {
            if (!err && data.Contents) {
                var uploadOneArchive = function (list, index) {
                    ...
                    if (index < list.length) {
                        uploadArchiveFromS3(bucket, list[index], vaultName, function (err, res){
                            ...
                            if (err) {
                                console.info('{"message":"error occurred while uploading ' + list[index] + '"}');
                            }
                        });
                    } else {
                ...
                if (data.Contents.length) {
                    ...
                    fileList.push("/" + data.Contents.Key);
                }
